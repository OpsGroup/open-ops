...
private:
	SSAForm* m_form;
	std::ostream* m_out_stream;
	OPS::Analysis::InductionAnalysis* mInduction;
public:
	using OutToC::visit;
	InductionToString(std::ostream& outStream, SSAForm& form, OPS::Analysis::InductionAnalysis& ind):OutToC(outStream),m_form(&form), m_out_stream(&outStream),   mInduction(&ind) {}
	void visit(ReferenceExpression& re)
	{
		OutToC::visit(re);
		if(m_form->getSubscript(&re))
...
int main()
{
	...
	{
		Frontend frontend;		
		const CompileResult& result = frontend.compileSingleFile("Samples/test.c");
...
//			writer.visit(frontend.getProgramUnit().getUnit(0));
			...
			SubroutineDeclaration* proutine = 0;

			OPS::Reprise::Declarations::SubrIterator subriter = decls.getFirstSubr();
			if ( subriter->getName() == "main" )
				proutine = &*subriter;
			else {
			...
			} while (subriter != decls.getLastSubr());
			}
			
			SubroutineDeclaration& subroutine = * proutine;
			...
			BlockStatement& bodyBlock = subroutine.getBodyBlock();
 			try
 			{
				...
				//ForStatement& loop = (bodyBlock.getFirst())->cast_to<ForStatement>();
				//ForStatement& loop = (++ploop.getBody().getFirst())->cast_to<ForStatement>();

				OPS::Analysis::InductionAnalysis indan(ssa);
