...
bool typeInfo(ArgInfo& var, TypeBase* type)
{
	TypeTraits traits;

	if (!traits.read(type) || traits.format != TypeTraits::Integer)
		return false;
	...
	TypeDeclaration* bitType)			// "bit" type
{
	...
	SubroutineManager man(
		new SubroutineDeclaration(
			new SubroutineType(
				handler.result.type,		// result type
	...
	for (argument i = args.begin(); i != args.end(); ++i)
		man.addParameter(i->type, i->name());

	// declare arguments as local variables	- this is required
	
	for (argument i = args.begin(); i != args.end(); ++i)
		man.addDeclaration(i->type, i->name(), false);	
	...
	Declarations& globals)
{
	...
	if (!typeInfo(handler.result, resType) ||
		!typeInfo(handlerArg, srcType))
		return NULL;
...
{
	for (int i = 0; i < expr.getArgumentCount(); i++)
		if (!isInt(expr.getArgument(i).getResultType()))
			return false;
...
bool canApply(const BasicCallExpression& expr)
{
	return
		...
		Utils::isUnsignedInt(expr.getArgument(0).getResultType()) &&
...
void apply(BasicCallExpression& expr)
{
	...
	SubroutineCallExpression* conv = createCoercionCall(
		expr.getArgument(1).getResultType(),
		expr.getArgument(0).getResultType(),
		getGlobals(expr));
...
bool canApply(const BasicCallExpression& expr)
{
	return
		allSRAccessible(expr) &&
		expr.getArgumentCount() == 2 &&
		isInt(expr.getArgument(0).getResultType()) &&
